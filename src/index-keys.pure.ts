/**
 * Pure index key generation for Hyperbee B-tree indexes.
 *
 ...
 *
 * Key format:
 *   link:hash:{hash}               → full LinkExpression JSON
 *   link:src:{source}:{hash}       → link hash (pointer)
 *   link:tgt:{target}:{hash}       → link hash (pointer)
 *   link:pred:{predicate}:{hash}   → link hash (pointer)
 *   link:auth:{author}:{hash}      → link hash (pointer)
 *   link:time:{timestamp}:{hash}   → link hash (pointer)
 *   meta:head                      → latest commit sequence
 *   meta:neighbourhood             → neighbourhood config
 */
